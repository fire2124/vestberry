...
CREATE TABLE IF NOT EXISTS `mysql`.`Status` (
  ...
  `Book_User_user_id` INT NOT NULL,
  PRIMARY KEY (`status_id`),
  INDEX `fk_Status_Book1_idx` (`Book_isbn` ASC, `Book_User_user_id` ASC) VISIBLE,
  UNIQUE INDEX `status_id_UNIQUE` (`status_id` ASC) VISIBLE,
  CONSTRAINT `fk_Status_Book1`
    FOREIGN KEY (`Book_isbn` , `Book_User_user_id`)
    REFERENCES `mysql`.`Book` (`isbn` , `User_user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
